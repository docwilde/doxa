...
# The peer-count regression that used to be pinned here -- issue #60,
# `renderPanel`'s peer-counting block lifted out of assets/mesh/mesh.js
# between two anchor lines and run in Node -- now lives in
# tests/test_mesh_page.py, which loads the real page in a real browser and
# clicks the real node. That was the right test to write when there was no
# way to execute the page; it is the wrong one to keep now that there is,
# because it depended on two lines of mesh.js keeping their exact text and
# would have gone quiet the moment either moved.
