...
"""A curated WRITE -- a proposal approved or rejected, a belief outcome
recorded, a belief retracted -- must leave the status bar's chips correct
afterwards, not just the store on disk. Reported alongside the memory
chip's startup/switch/resume defect (tests/test_memory_chip_slug.py):
none of the four write actions triggered a status refresh at all, so a
chip stayed exactly as stale as whatever it last happened to read.
...
(``fake.approved``/``fake.rejected``/``fake.outcomes_recorded``/
``fake.retracted``) prove the write itself still happens exactly once, on
the CONFIRMING selection, never the arming one. The chip assertion is
therefore a direct read of ``_refresh_status``'s own effect: if the
action's completion is what triggers the reread, the chip changes
IMMEDIATELY after the confirming selection and never before it."""

from __future__ import annotations

import pytest

from doxa.ui.beliefs import BeliefRow
...
# -- via the status-bar's quick pending picker -------------------------------
...
# -- via the status-bar's quick beliefs picker -------------------------------
...
# -- the same actions, from the full beliefs browser -------------------------


@pytest.mark.asyncio
async def test_the_browsers_own_retract_also_moves_the_belief_chip(
    monkeypatch, tmp_path,
):
    """The beliefs BROWSER (a separate tab, doxa/ui/beliefs.py) drives the
    same engine method through its own row -- and reported alongside the
    dropdown's version, its retract never nudged the status bar either.
    One test on this surface, not four: the write/chip pairing per action
    is already pinned above, and BeliefsBrowserTab.retract's only new
    behaviour to prove is that IT ALSO calls back into the owning pane."""
    retracted_flag = {"done": False}
    from doxa.session import chips as chips_mod
    ...
    async with app.run_test(size=(180, 48)) as pilot:
        ...
        await pilot.press("d")  # arms retract
        await pilot.pause()
        assert fake.retracted == []
        await pilot.press("d")  # confirms
        await _wait(pilot, lambda: fake.retracted == [7])
