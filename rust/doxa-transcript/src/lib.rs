...
fn invalid() -> io::Error { io::Error::new(io::ErrorKind::InvalidInput, "unsafe transcript path or identifier") }
fn bad_data() -> io::Error { io::Error::new(io::ErrorKind::InvalidData, "invalid JSON record") }

/// Matches doxa.identity's `[0-9A-Za-z][0-9A-Za-z-]{0,127}`.
pub fn valid_session_id(id: &str) -> bool {
    !id.is_empty() && id.len() <= 128 && id.bytes().enumerate().all(|(i, b)|
        b.is_ascii_alphanumeric() || (i > 0 && b == b'-'))
}

fn valid_slug(slug: &str) -> bool {
    !slug.is_empty() && slug.len() <= 255 && slug != "." && slug != ".." &&
        !slug.contains('/') && !slug.contains('\\') && !slug.chars().any(char::is_control)
}
...
fn safe_root(path: &Path) -> io::Result<()> {
    ...
    for component in path.components() {
        if matches!(component, Component::ParentDir | Component::CurDir) { return Err(invalid()); }
        prefix.push(component);
        if fs::symlink_metadata(&prefix)?.file_type().is_symlink() { return Err(invalid()); }
    }
    owned_dir(path)
}
...
fn open_read(path: &Path) -> io::Result<File> {
    ...
    let file = opts.read(true).custom_flags(libc::O_NOFOLLOW | libc::O_NONBLOCK).open(path)?;
    checked_file(&file)?;
    Ok(file)
}
...
fn scrub_value(value: &mut Value, scrub: &impl Fn(&str) -> String) {
    match value {
        ...
        Value::Array(items) => for item in items { scrub_value(item, scrub); },
        Value::Object(fields) => for item in fields.values_mut() { scrub_value(item, scrub); },
        _ => {}
    }
}

pub struct TranscriptStore { dir: PathBuf, session_id: String }
...
impl TranscriptStore {
    ...
    pub fn new(projects_dir: &Path, slug: &str, session_id: &str) -> io::Result<Self> {
        if !valid_slug(slug) || !valid_session_id(session_id) { return Err(invalid()); }
        safe_root(projects_dir)?;
        let dir = projects_dir.join(slug);
        if !dir.exists() { fs::create_dir(&dir)?; }
        owned_dir(&dir)?;
        Ok(Self { dir, session_id: session_id.to_owned() })
    }

    pub fn transcript_path(&self) -> PathBuf { self.dir.join(format!("{}.jsonl", self.session_id)) }
    pub fn thread_path(&self) -> PathBuf { self.dir.join(format!("{}{}", self.session_id, THREAD_SUFFIX)) }

    /// Append one original record with the Python engine override, after scrubbing every string value.
    pub fn append(&self, mut record: Value, engine: &str, scrub: impl Fn(&str) -> String) -> io::Result<()> {
        owned_dir(&self.dir)?;
        ...
        if bytes.len() > MAX_TRANSCRIPT_BYTES { return Err(bad_data()); }
        bytes.push(b'\n');
        let mut opts = OpenOptions::new();
    ...
    pub fn read_records(&self) -> io::Result<Vec<Value>> {
        ...
        let mut raw = Vec::new();
        file.take(MAX_TRANSCRIPT_BYTES as u64).read_to_end(&mut raw)?;
        if start > 0 {
            match raw.iter().position(|b| *b == b'\n') {
                Some(pos) => { raw.drain(..=pos); },
                None => return Ok(Vec::new()),
            }
        }
        Ok(raw.split(|b| *b == b'\n').rev().take(MAX_TRANSCRIPT_LINES).collect::<Vec<_>>()
            .into_iter().rev().filter_map(|line| serde_json::from_slice::<Value>(line).ok())
            .filter(Value::is_object).collect())
    }
    ...
    pub fn read_thread(&self) -> io::Result<Option<Value>> {
        ...
        if file.metadata()?.len() > MAX_METADATA_BYTES { return Err(bad_data()); }
        let mut raw = Vec::new();
        file.read_to_end(&mut raw)?;
        let value: Value = serde_json::from_slice(&raw).map_err(|_| bad_data())?;
        if !value.is_object() { return Err(bad_data()); }
        Ok(Some(value))
    }
    ...
    pub fn recorded_thread_id(&self) -> io::Result<Option<String>> {
        ...
        match self.read_thread() {
            Ok(value) => Ok(value.and_then(|v| v.get("thread_id")?.as_str().filter(|s| !s.is_empty()).map(str::to_owned))),
    ...
    pub fn write_thread(&self, updates: Map<String, Value>, scrub: impl Fn(&str) -> String) -> io::Result<()> {
        owned_dir(&self.dir)?;
        let mut base = self.read_thread()?.and_then(|v| v.as_object().cloned()).unwrap_or_default();
        base.extend(updates);
        if !base.get("thread_id").and_then(Value::as_str).is_some_and(|s| !s.is_empty()) { return Err(bad_data()); }
