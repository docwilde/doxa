...
impl UiStateStore {
    ...
    pub fn new(home: &Path, scope_key: &str, machine_id: &str) -> io::Result<Self> {
        if scope_key.is_empty() || machine_id.is_empty() {
            return Err(io::Error::new(io::ErrorKind::InvalidInput, "missing scope or machine id"));
        }
        ...
        Ok(Self { path, scope_key: scope_key.into(), record, writable_layout })
    }
    ...
    pub fn restore(&self, app: &mut App, live_ids: &[String]) -> bool {
        ...
        let tabs: Vec<_> = record.tabs.iter().filter(|t| live.contains(t.session_id.as_str())).collect();
        if tabs.is_empty() { return false; }
        let ids: Vec<String> = tabs.iter().map(|t| t.session_id.clone()).collect();
        let active = record.active_session_id.as_deref().filter(|id| ids.iter().any(|s| s == id));
        ...
        } else if let Some(trees) = layout.and_then(|l| l.get("trees")).and_then(Value::as_array) {
            let chosen = trees.iter().find(|tree| active.is_some_and(|id| tree_ids(tree).contains(id)))
                .or_else(|| trees.first());
            if let Some((projected, orientation, weight)) = chosen.and_then(|tree| parse_legacy_tree(tree, &ids)) {
                groups = projected;
                split = orientation;
                percent = weight;
            } else {
        ...
        if let Some(rust_ui) = record.raw.get("rust_ui") {
            if let Some(visible) = rust_ui.get("rail_visible").and_then(Value::as_bool) { app.rail_visible = visible; }
    ...
    pub fn save(&mut self, app: &App) -> io::Result<()> {
        ...
        for group in &app.groups {
            for id in &group.tabs {
                if !valid_session_id(id) { return Err(io::Error::new(io::ErrorKind::InvalidInput, "invalid session id")); }
                if seen.insert(id.as_str()) {
                    let old = self.record.as_ref().and_then(|r| r.tabs.iter().find(|t| t.session_id == *id));
                    tabs.push(Tab { session_id: id.clone(), pinned_name: old.and_then(|t| t.pinned_name.clone()), cwd: old.and_then(|t| t.cwd.clone()) });
                }
            }
        }
        ...
        let trees: Vec<Value> = app.groups.iter().filter_map(|g| g.tabs.get(g.active).map(|id| leaf(&record, id))).collect();
        ...
        let object = layout.as_object_mut().ok_or_else(|| io::Error::new(io::ErrorKind::Unsupported, "unknown layout format"))?;
        object.insert("groups".into(), group_tree);
        object.insert("trees".into(), Value::Array(trees));
        record.raw.insert("rust_ui".into(), json!({"rail_visible":app.rail_visible,"rail_width":app.rail_width.clamp(12,44)}));
        // Remove dangling collection membership before changing the flat list.
        if let Some(rows) = record.raw.get_mut("collections").and_then(Value::as_array_mut) {
            for row in rows {
...
fn leaf(record: &TabSet, id: &str) -> Value {
    ...
    let mut row = record.raw.get("layout").and_then(|layout| layout.get("groups"))
        .and_then(|root| find_leaf(root, id)).cloned()
        .or_else(|| record.raw.get("layout").and_then(|layout| layout.get("trees"))
            .and_then(Value::as_array).and_then(|trees| trees.iter().find_map(|root| find_leaf(root, id))).cloned())
        .unwrap_or_else(|| json!({"kind":"leaf","session_id":id}));
    if let Some(name) = old.and_then(|t| t.pinned_name.as_ref()) { row["pinned_name"] = json!(name); }
    if let Some(cwd) = old.and_then(|t| t.cwd.as_ref()) { row["cwd"] = json!(cwd); }
    row
}
fn find_leaf<'a>(node: &'a Value, id: &str) -> Option<&'a Value> {
    if node.get("kind").and_then(Value::as_str) == Some("leaf") && node.get("session_id").and_then(Value::as_str) == Some(id) { return Some(node); }
    for key in ["tabs", "children"] {
        if let Some(rows) = node.get(key).and_then(Value::as_array) {
            for row in rows { if let Some(found) = find_leaf(row, id) { return Some(found); } }
        }
    }
    None
}
fn tree_ids(raw: &Value) -> HashSet<&str> {
    ...
    fn walk<'a>(node: &'a Value, ids: &mut HashSet<&'a str>) {
        if let Some(id) = node.get("session_id").and_then(Value::as_str) { ids.insert(id); }
        for key in ["tabs", "children"] {
            if let Some(rows) = node.get(key).and_then(Value::as_array) { for row in rows { walk(row, ids); } }
        }
    }
    walk(raw, &mut ids);
    ids
...
fn weight(raw: &Value) -> u16 {
    raw.get("weights").and_then(Value::as_array).and_then(|w| w.first()).and_then(Value::as_f64)
        .filter(|w| w.is_finite() && *w > 0.0 && *w < 1.0).map(|w| (w * 100.0).round() as u16).unwrap_or(50).clamp(20,80)
}
fn supported_layout(raw: &serde_json::Map<String, Value>) -> bool {
    let Some(layout) = raw.get("layout") else { return true; };
    let Some(layout) = layout.as_object() else { return false; };
    if layout.get("kind").and_then(Value::as_str) != Some("tabs") { return false; }
    if let Some(groups) = layout.get("groups") { return supported_group_tree(groups); }
...
fn supported_group_tree(node: &Value) -> bool {
    match node.get("kind").and_then(Value::as_str) {
        Some("group") => node.get("tabs").and_then(Value::as_array).is_some_and(|tabs| tabs.iter().all(|leaf| leaf.get("kind").and_then(Value::as_str) == Some("leaf"))),
        Some("split") => parse_orientation(&node["orientation"]).is_some() && node.get("children").and_then(Value::as_array).is_some_and(|children| children.len() == 2 && children.iter().all(|child| child.get("kind").and_then(Value::as_str) == Some("group") && supported_group_tree(child))),
        _ => false,
    }
}
fn supported_legacy_tree(node: &Value) -> bool {
    match node.get("kind").and_then(Value::as_str) {
        Some("leaf") => true,
        Some("split") => parse_orientation(&node["orientation"]).is_some() && node.get("children").and_then(Value::as_array).is_some_and(|children| children.len() == 2 && children.iter().all(|child| child.get("kind").and_then(Value::as_str) == Some("leaf"))),
        _ => false,
    }
}
fn group_tabs(raw: &Value, live: &[String]) -> Option<PaneGroup> {
    if raw.get("kind")?.as_str()? != "group" { return None; }
    let mut group = empty_group();
    for row in raw.get("tabs")?.as_array()? {
        let Some(id) = row.get("session_id").and_then(Value::as_str) else { continue; };
        if live.iter().any(|s| s == id) && !group.tabs.iter().any(|s| s == id) { group.tabs.push(id.into()); }
    }
    let saved_active = raw.get("active").and_then(Value::as_u64).unwrap_or(0) as usize;
    if let Some(id) = raw.get("tabs").and_then(Value::as_array).and_then(|rows| rows.get(saved_active)).and_then(|row| row.get("session_id")).and_then(Value::as_str) {
...
fn parse_groups(raw: &Value, live: &[String]) -> Option<([PaneGroup; 2], Split, u16)> {
    if raw.get("kind")?.as_str()? == "group" { return Some(([group_tabs(raw, live)?, empty_group()], Split::Vertical, 50)); }
    if raw.get("kind")?.as_str()? != "split" { return None; }
    let children = raw.get("children")?.as_array()?;
    if children.len() != 2 { return None; }
    Some(([group_tabs(&children[0], live)?, group_tabs(&children[1], live)?], parse_orientation(&raw["orientation"])?, weight(raw)))
}
fn parse_legacy_tree(raw: &Value, live: &[String]) -> Option<([PaneGroup; 2], Split, u16)> {
    ...
    for (index, child) in children.iter().enumerate() {
        if child.get("kind")?.as_str()? != "leaf" { return None; }
        let id = child.get("session_id")?.as_str()?;
        if live.iter().any(|s| s == id) { groups[index].tabs.push(id.into()); }
    }
