...
fn persisted_snapshot_attaches_at_hello_head_without_ring_replay() {
    ...
    // Python uses plain append, so a normal transcript may be 0644.
...
#[test]
fn special_or_linked_transcript_falls_back_to_ring_replay() {
    let dir = std::env::temp_dir().join(format!(
        "doxa-rust-snapshot-{}-{}", std::process::id(), NEXT_SOCKET.fetch_add(1, Ordering::Relaxed)
    ));
    std::fs::create_dir(&dir).unwrap();
    let regular = dir.join("regular.jsonl");
    std::fs::write(&regular, b"{\"type\":\"user\"}\n").unwrap();
    std::fs::set_permissions(&regular, std::fs::Permissions::from_mode(0o600)).unwrap();
    let fifo = dir.join("fifo.jsonl");
    let fifo_name = std::ffi::CString::new(fifo.as_os_str().as_bytes()).unwrap();
    assert_eq!(unsafe { libc::mkfifo(fifo_name.as_ptr(), 0o600) }, 0);
    let linked = dir.join("linked.jsonl");
    symlink(&regular, &linked).unwrap();

    for transcript in [&fifo, &linked] {
        let server_path = transcript.clone();
        let (path, task) = socket(move |mut stream| {
            send(&mut stream, json!({"type":"hello","proto":1,"session_id":"session-1",
                "cwd":"/tmp","model":null,"engine":"doxa","next_seq":700,
                "transcript_path":server_path,"transcript_bytes":1}));
            let mut reader = BufReader::new(stream.try_clone().unwrap());
            assert_eq!(line(&mut reader), json!({"type":"attach","cursor":null}));
        });
        let (_, snapshot) = DaemonClient::connect_for_restore(&path).unwrap();
