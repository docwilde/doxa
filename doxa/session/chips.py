...
class PaneChipsMixin:
    ...
    async def _record_belief_outcome(self, belief: dict, event: str) -> None:
        ...
        # Reached from BOTH the dropdown menu and the inline row action
        # (:meth:`_dispatch_belief_row_action`) -- one write function,
        # one refresh, regardless of which row control fired it.
    ...
    async def _resolve_pending(self, item: "Any", action: str) -> None:
        ...
        # Reached from BOTH the dropdown menu and the inline row action
        # (:meth:`_dispatch_pending_row_action`).
        self._refresh_status()
        if action == "approve":
            await self._system(
