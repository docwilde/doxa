...
both fixed in v0.57.0 after `doxa launcher install` from a current
...
#: What :func:`stale_entry` reports for an entry written before v0.57.0,
...
def installed_exec() -> "Path | None":
    ...
    doxa.cli``, so it is the thing to check for existence. A pre-v0.57.0
...
def stale_entry() -> "str | None":
    ...
    * an entry with no version key -> :data:`UNVERSIONED`. Every entry
      written before v0.57.0 is one of these, including the one that
