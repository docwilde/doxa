...
| skills (`skills/*/SKILL.md`) | **adopted** | Same risk class `cli_isolation.ensure_skills_snapshot` already carries wholesale for the operator's own learned skills — read by the model only when it reaches for that skill. This widens the SOURCE (a plugin's bundled skills, not reached by that symlink) without widening the RISK. |
