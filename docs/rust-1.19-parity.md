...
| `/movepane`, `/collection`, `/rename`, `/cd`, `/clear` | `/movepane [1|2]` moves an active tab between two groups while retaining a source tab; named `/collection new|rename|delete|add|remove` orders the rail; `/rename` local; `/cd <path>` opens a verified new-session directory; `/clear` replaces an idle session only after a durable tabset swap | Final-tab movement and replacement when no writable complete tabset is available |
