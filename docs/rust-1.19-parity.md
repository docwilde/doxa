...
Rust branch's behavior at `v2.0.0-alpha.20`; it is a release gate, not a
...
| `/movepane`, `/collection`, `/rename`, `/cd`, `/clear` | `/movepane [1|2]` moves an active tab between two groups while retaining a source tab; `/rename` local | Final-tab movement, collection state and safe new-session directory controls |
