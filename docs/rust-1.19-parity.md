...
Rust branch's behavior at `v2.0.0-alpha.19`; it is a release gate, not a
...
| Window and prompt | Two split panes, persistent named tabs, grouped rail, per-session drafts, mouse selection and resize, inline questions, Markdown with Ctrl+click HTTP(S) links, live tool detail and restored Claude tool detail, expandable reasoning, processing spinner inside transcript; active-session repo/worktree chip | More than two pane groups, moving a tab across groups and sidebar collections; durable Codex tool details |
