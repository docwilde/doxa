...
| Window and prompt | Two split panes, persistent named tabs, guarded `/movepane` tab transfer, named collections and grouped rail, per-session drafts, mouse selection and resize, inline questions, Markdown with Ctrl+click HTTP(S) links, live and newly restored Claude/Codex tool detail, expandable reasoning, processing spinner inside transcript; active-session repo/worktree chip | More than two pane groups, moving a final tab out of a pane; historic Codex transcripts cannot recover details that were never stored |
