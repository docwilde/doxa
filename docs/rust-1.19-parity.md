...
| Window and prompt | Two split panes, persistent named tabs, grouped rail, per-session drafts, mouse selection and resize, inline questions, Markdown, live tool detail and restored Claude tool detail, expandable reasoning, processing spinner inside transcript; active-session repo/worktree chip | More than two pane groups, moving a tab across groups and sidebar collections; durable Codex tool details |
