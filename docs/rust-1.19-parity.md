...
| Operational UI | Native doctor and install launcher; CLI `setup`, `auth status`, Claude Code `plugins` inventory, and `settings` read/set/unset for native linger and worktree preferences; CLI `update`; per-session usage/context detail panels | Interactive setup/settings/login/logout/plugin adoption and provider context component breakdown |
...
| `/login`, `/logout`, `/settings`, `/setup`, `/doctor`, `/update` | CLI doctor, update, setup report and auth-status probes, native `doxa settings` read/set/unset for linger and worktree preferences | Interactive operations and broader settings changes |
