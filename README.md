...
- [`docs/plans/peer-publishing.md`](docs/plans/peer-publishing.md) — **what a session publishes about itself to same-repo peers.** Nothing implemented. `PeerInfo` carries no `provider`/`model`/`engine` today; the spec argues which of those are safe to add (self-reported, therefore untrusted, therefore display-only) and why a capability field like context window is not.
